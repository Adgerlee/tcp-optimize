#!/bin/bash
# ===================================================================
# 🚀 TCP 优化脚本 | 支持跨境场景 | BBR + CAKE | 完全兼容 dash/bash
# 作者：Adger
...
rmem_max=$(echo "$bdp_bytes * 2" | bc -l)
rmem_max=$(printf "%.0f" "$rmem_max")  # 修复：浮点 → 整数
...
echo -e "${GREEN}✅ TCP 优化已生效（BBR + CAKE）${NC}"
